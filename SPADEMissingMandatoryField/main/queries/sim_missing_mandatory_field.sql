...
select distinct sim.title,
                trim(replace(replace(assigneeidentity,'kerberos:',''),'@ANT.AMAZON.COM', '')) AS Assignee,
                -- date_diff('day'::text, TO_DATE(createdate, 'YYYY-MM-DD'), getdate())
                --  - date_diff('week'::text, TO_DATE(createdate, 'YYYY-MM-DD'), getdate()) * 2 as business_days,
                sim.issueurl,
                case when label_customer is null then 'Yes'
                     else 'No'
                end                      as if_missing_label_customer,
                case when coalesce(estimatedcompletiondate, resolveddate) is null and sprint_label_count > 0
                          and (number_days_resolved is null or number_days_resolved < 15) then 'Yes'
                     else 'No'
                end                      as if_missing_ets_completion_day
                --sim.issueurl,
from (select title,
             issueurl,
             assignedfolderid,
